...
function SceneObjects({ theme }) {
  ...
  useFrame(({ clock }) => {
    ...
    // Rotate the complete group safely
    if (group.current) {
      group.current.rotation.y = elapsedTime * 0.1;
    }

    // Rotate the wireframe sphere safely
    if (wireframeSphere.current) {
      wireframeSphere.current.rotation.x = elapsedTime * 0.11;
    }
  });
    ...
    <group ref={group}>
      ...
      >
        ...
        {/* Changed from meshBasicMaterial to meshStandardMaterial
            because emissive/emissiveIntensity are supported here */}
      ...
      {[...Array(7)].map((_, i) => (
        ...
        >
          ...
          <meshStandardMaterial
            ...
            emissiveIntensity={isLight ? 0.6 : 0.8}
          />
        </mesh>
      ))}
    </group>
